...
import { handleMappingFieldRequiredForAccount, handleMappingFieldRequiredForFileData } from '../../../utils/MappingField'
import { queryFileDataDetailsById, updatFileDataDetails } from '../../../api/fileData'
...
export default class Details extends HocComponent<Props> {
  ...
  queryData() {
    queryFileDataDetailsById(this.props.id).then((res) => {
      const detailsData = handleMappingFieldRequiredForFileData(res)
      this.setNewState({
        dataType: detailsData,
        formSubmitLoading: false,
      })
  ...
  editData(data: DetailsData[]) {
    ...
    updatFileDataDetails(this.props.id, data).then(res => {
  ...
  renderExtra() {
    return (
      <div>
        {
          this.state.allowEditFlag ? this.props.isShowAction(<Button type='link' onClick={() => this.allowEdit(false)}>编辑</Button>) : (
            <div>
              <span><Button type='link' onClick={() => this.allowEdit(true)}>取消编辑</Button></span>
            ...
            children={
              this.state.dataType.length ? (
                <ParentContext.Consumer>
                  {
                    (value: Pick<DataType, "id" | "name"> & Record<"plate", string>) => <InfoCom
                    ref={this.infoCom}
                    data={this.state.dataType}
                    allowEditFlag={this.state.allowEditFlag}
                    loading={this.state.formSubmitLoading}
                    supply={value}
                    parentMethod={(data: DetailsData[]) => this.editData(data)}
                  ></InfoCom>
                  }
                </ParentContext.Consumer>
              ) : (<Skeleton active/>)
